...
def _root_path():
  return os.path.realpath(os.path.join(BUILD_PATH, '..'))


def _goto_root(func):
  @wraps(func)
  def goto_func(*args, **kwargs):
    with fab.lcd(_root_path()):
...
def production():
  fab.env.git = _deploy_git_factory()(
    remote_repository_path='',  ################## add path to remote server
    release_branch='production',
  )

  _deploy_base_env()
  fab.env.hosts = ['hostname'] ############# add ssh hostname
...
def staging():
  ...
  _deploy_base_env()
  fab.env.hosts = ['jan.mittwald'] ############# add ssh hostname
